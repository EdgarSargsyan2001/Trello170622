...
export default memo(function Section({ sec,aa, setInfoModal }: sectionPrpos) {
  console.log('section up');
  const dispatch = useDispatch();
  console.log(aa.current)

  const [{ canDrop,isOver }, RefDrop,dragPreview] = useDrop(() => ({
    accept:'task',
  ...
  return (
    <div
      ...
      // ref={RefDrop}
    >
      <h3 ref={RefDrop} className="secTitle">
        Title<p>{sec?.title}</p>
      </h3>

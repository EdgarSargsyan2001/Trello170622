...
import {changeTitle,changeDesc,changeCl1,changeCl2} from '../../features/inputs/inputsSlice'
...
function Modal({setOpenModal,modalType}:{setOpenModal:(bul:boolean)=>void,modalType:string}){
    ...
    const {title,desc,color1,color2} = useSelector((state:State) => state.inputs)
        ...
        <div style={modalType==='task'?modalStyle:{}} className="modal">
            ...
            <span className='modalSpan'></span>
                ...
                <input 
                    type='color' 
                    value={color1} 
                    onChange={(e)=>dispatch(changeCl1(e.target.val))}
                />

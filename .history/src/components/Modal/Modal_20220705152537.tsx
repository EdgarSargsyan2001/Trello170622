...
import { addSec, addTask,editSec } from '../../features/section/sectionSlice';
import './Modal.css';
...
function Modal({ setInfoModal, infoModal }: ModalProps) {
  ...
  const modalBtnHandel = () => {
    if (infoModal.modalType === 'task') {
      ...
      EditSec()
    } else if (infoModal.modalType === 'edit') {
      EditTask()
    }
  ...
  return (
    ...
    >
      ...
      <button className="modalBtn" onClick={modalBtnHandel}>
        {(infoModal.modalType === 'section' || infoModal.modalType === 'task')?"Add":'Edit'}
      </button>
    </div>
  );
}

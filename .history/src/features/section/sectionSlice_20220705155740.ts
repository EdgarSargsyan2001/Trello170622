...
export const sectionSlice = createSlice({
  ...
  reducers: {
    ...
    editTask: (state, action) => {
     return state.map((sec: SecObj):any => {
        const newSec = sec.tasks.filter(
          (task: TaskObj) => task.id !== action.payload.taskId,
        );
        return newSec
      });
      console.log(action)
...
export const { addSec, addTask, closeSec, closeTask, dragDrop, editSec,editTask } =
